...
def log_audit(action: str, details: str = None, username: str = None):
    """Record an audit log entry. Works both with and without an active request context."""
    uid = None
    uname = username
    ip = None
...
def log_audit(action: str, details: str = None, username: str = None):
    try:
        u = current_user if hasattr(current_user, "is_authenticated") and current_user.is_authenticated else None
    except Exception:
        u = None
    entry = AuditLog(
...
def manage():
    ...
    # Handle creation actions based on submitted form 'action' field
    if request.method == "POST":
        ...
        # --- Function management ---
            ...
            if not fid:
                ...
                if not fobj:
                    ...
                    if not new_name:
                        flash("Function name is required.", "warning")
                    elif not is_json(new_tmpl or "{}"):
                        flash("Function template must be valid JSON.", "warning")
                    else:
                        # Check name conflicts (if changed)
